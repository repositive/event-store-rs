...
use event_store::testhelpers::{pg_create_random_db, TestCounterEntity};
use event_store::{
...
///
/// It requires a running Postgres database. You can start one using `docker-compose up postgres` in
/// the project repo.
fn pg_store_pg_cache(c: &mut Criterion) {
    ...
    c.bench_function("exercise pg store and cache", move |b| {
        b.iter(|| {
